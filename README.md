...
**Fear&Greed 오실레이터(S&P500/NASDAQ)의 VIX·금리를 FRED 대신 Yahoo 로 전환** —
`_compute_fear_greed_pair()` 가 쓰던 FRED `VIXCLS`/`DGS10`/`DGS5` 를 Yahoo `^VIX`/`^TNX`/`^FVX`
로 바꿨습니다. `^VIX` 는 FRED `VIXCLS` 와 사실상 동일한 값(둘 다 CBOE 원천)이고,
`^TNX`/`^FVX` 는 CBOE 지수라 재무부 공식치(DGS10/DGS5)와 소수점 둘째자리 정도
차이가 날 수 있지만 정규화해 합성하는 이 계산 목적상 무시할 수준입니다.

전환하면서 확인한 사실: **FRED 의 `VIXCLS`·`DGS10` 은 2026-09-18 이후로 원본 자체가
갱신을 멈춘 상태**입니다(직접 `cosd` 로 조회해 확인 — 이건 GitHub Actions 의
연결 타임아웃 문제와는 별개로, FRED 쪽 해당 계열의 진짜 데이터 지연입니다).
...
**(후속 수정) `rate_hy_combo` 의 10년물도 Yahoo `^TNX` 로 통일** — 단독 카드와 값이
어긋나 보이는 문제를 해결하기 위해 `rate_hy_combo()` 의 10년물도 FRED `DGS10`
대신 `index_series(S.FG_DGS10, ...)` 로 바꿔, 이제 단독 카드와 정확히 같은 값을
씁니다. 하이일드 스프레드(`BAMLH0A0HYM2`)는 여전히 Yahoo 에 대응 티커가 없어
그대로 FRED 를 씁니다.

**남은 제약**: 확인해보니 하이일드 스프레드도 `VIXCLS`/`DGS10` 과 같은 이유로
FRED 원본이 **09-18 이후 갱신이 멈춘 상태**입니다(세 번째 사례) — 이건 Yahoo
대안이 없어 우회 불가능합니다. 그래서 통합 차트는 10년물 쪽이 신선해져도(Yahoo,
09-21까지) 스프레드 쪽이 09-18에 멈춰 있는 한 **날짜 교집합이 09-18까지로
제한**됩니다. FRED 가 이 계열을 다시 갱신하면 자동으로 풀립니다.
